        ...
        <div class="dialog_frame a-5-bg" v-if="gridShowFlag" ref="dialog">
          <div class="dialog_frame_head a-4-bg">
            <span class="dialog_frame_head_text b-1-cl">
              {{
                symbol
                  ? getCoinShowName(symbol[0], coinList) +
                    "/" +
                    getCoinShowName(symbol[1], coinList)
                  : ""
              }}
              <!-- 运行时长 -->
              ...
              {{
                type.toString() === "0"
                  ? setTime(gridData.startTime, gridData.endTime)
                  : setTime(gridData.startTime)
              }}</span
            >
              ...
              <div class="detail_content">
                ...
                <div class="detail_content_item">
                  ...
                  <div class="detail_content_item_val">
                    ...
                    {{ symbol ? getCoinShowName(symbol[1], coinList) : " " }}
                    {{
                      gridData && gridData.configParamMap && gridData.configParamMap.totalBaseAmount
            ...
            <div class="dialog_being" v-if="transactionType === 2">
              <div class="being_conent">
                <vue-scroll>
                  ...
                  />
                  <c-pagination
                    v-if="pagination.total / pagination.pageSize > 1"
...
<script>
import {
  colorMap, formatTime, fixD, getCoinShowName,
    ...
    columns() {
      ...
      if (this.transactionType === 3) {
        return [
          ...
          {
            title: `${this.$t("gridTrade.dialog11")}(${
              this.symbol ? getCoinShowName(this.symbol[1], this.coinList) : ""
            })`, // '利润'
          },
        ];
      }
            ...
            if (data.data && data.data && data.data.list.length) {
              data.data.list.forEach((item, i) => {
                ...
                if (item.buyOrder && !item.sellOrder) {
                  subData.push({
                    typeText:
                      item.buyOrder.orderSide === "BUY"
                        ? this.$t("trade.buy")
                        : this.$t("trade.sell"),
                    balanceSymbol: this.symbol
                      ? getCoinShowName(this.symbol[1], this.coinList)
                      : "",
