...
<template>
  ...
  <div class="trade-box trade-box-ex-container b-2-cl">
    ...
    <div class="title-block a-3-bd-c" style="padding: 0px 16px">
      <span
        class="tab"
    ...
    <div class="trade-form trade-grid" style="padding: 0px" v-else-if="transactionType === 4">
      <template>
        ...
        <div class="trade-block" style="margin-top: 16px">
          <div
            class="form-block"
            style="
              height: 370px;
        ...
        >
          <div class="top-mag">
            ...
            <span class="m-account">
              ...
              <span class="coinNum">{{ currenTaccount.unitsAccoubt }} {{ symbolUnit.units | getCoinShowName(coinList) }}</span>
            </span>
            ...
            <!--  >-->
            <!--  &lt;!&ndash; 改，2022.10.19，新增style &ndash;&gt;-->
            <!--  <span class="lovText" style="color: #0c9"-->
            <!--    >{{ currenTaccount.unitsAccoubt }}-->
            <!--    {{ symbolUnit.units | getCoinShowName(coinList) }}-->
            <!--  </span>-->
            <!--</span>-->
          </div>
          <!-- 改，2022.11.22，新增inputStyle -->
          <!-- 改，2022.12.02，取消inputStyle -->
          <trade-input
          ...
          <!--  </span>-->
          <!--</div>-->
          <div class="m-percentage" :style="{ '--width': `${ 330 / perArr.length }px` }">
            <span
              v-for="item in perArr"
              :key="item"
              :class="{ 'active': perBuy === item, prevItem: item < perBuy }"
              @click="setPerNumber('buy', item)"
              class="item"
              :title="`${item}%`"
            ></span>
        ...
        >
          <div class="top-mag">
            ...
            <span class="m-account">
              ...
              >{{ $t("manageFinances.goRecharge") }}</span>
              <span class="coinNum">{{ currenTaccount.symbolAccoubt }} {{ symbolUnit.symbol | getCoinShowName(coinList) }}</span>
            </span>
            <!--<span class="lov b-1-cl">-->
            <!--  <c-button-->
            <!--    type="text"-->
          ...
          <div class="m-percentage" :style="{ '--width': `${ 330 / perArr.length }px` }">
            <span
              v-for="(item, index) in perArr"
              :key="index + 'sell'"
              :class="{ 'active': perSell === item, prevItem: item < perSell }"
              @click="setPerNumber('sell', item)"
              class="item"
              :title="`${item}%`"
            ></span>
    ...
    >
      <div class="m-info">
        ...
        <div class="item">
          {{ dialogContent.symbolCurrent }}<span class="type" :class="{ buy: dialogContent.side == 'BUY', sell: dialogContent.side == 'SELL'  }">({{ dialogContent.side }})</span>
        </div>
      </div>
      <div class="m-info">
      ...
      <div class="m-info">
        ...
        <div class="item" style="color: #FF0000">{{ dialogContent.price }}</div>
      </div>
      <div class="m-info">
        <div class="item">{{ $t("futures.coFlowingWater.amount") }}</div>
        <div class="item" style="color: #FF0000">{{ dialogContent.volume }}</div>
      </div>
      <div class="m-btnGroup">
        <div class="btn" style="margin-right: 20px;" @click="handlerClose">{{ $t('components.dialog.closeText') }}</div>
        <div class="btn active" :class="{confirmLoading: confirmLoading}" @click="handlerConfirm">{{ $t('components.dialog.confirmText') }}</div>
      </div>
    </c-dialog>
  </div>
</template>
  ...
  computed: {
    ...
    gridbuttosContent() {
      return {
        gridButton: {
          // 开启网格交易
          text: this.isLogin ? this.$t("gridTrade.btn1") : this.$t("trade.loginReg"), // '登录/注册',
          class: this.isLogin ? "u-1-bg buyBtn u-16-cl" : "u-8-bg buyBtn u-16-cl",
        },
        // 开启AI网格策略
        gridAiButton: {
          text: this.isLogin ? this.$t("gridTrade.btn2") : this.$t("trade.loginReg"), // '登录/注册',
          class: this.isLogin ? "u-1-bg buyBtn u-16-cl" : "u-8-bg buyBtn u-16-cl",
        },
      };
    },
