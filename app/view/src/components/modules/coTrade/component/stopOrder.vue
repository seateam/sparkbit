  ...
  >
    <div class="stopOrderMode-box">
      <table class="positionLis-table">
        ...
        <tbody class="tbody" v-if="dataInfo && dataInfo.sideClass">
          <tr class="a-3-bd">
            ...
            <td>
              <div>
                <p :class="dataInfo.sideClass.color">
                  <template v-if="dataInfo.orderSide === 'SELL'">-</template>
            ...
            <!-- 成本价 -->
            <td>
              <div>{{ thousandsComma(dataInfo.openAvgPrice) }}</div>
            </td>
            <!-- 最新价格-->
            <td>
              <div>
            ...
            <!-- 强平价格 -->
            <td>
              <div>{{ thousandsComma(dataInfo.reducePrice) }}</div>
            </td>
          </tr>
        </tbody>
      </table>
      <div class="block-title">
        ...
        <c-button height="26px" :disabled="!takeProfitCount" paddingW="6px" @click="cancelOrder(1)">
          <!-- 撤销所有止盈单 -->
          {{ lanText.text2 }}
          {{ takeProfitCount }}
        </c-button>
        ...
        <div class="form-info">
          ...
          <div class="info-item a-4-bg">
            ...
            <p class="text2 a-5-bd">
              <span :class="returnColor(takeAmountRate)">{{ takeAmountRate }}</span> %
            </p>
          </div>
        </div>
      </div>
      <div class="block-title">
        ...
        <c-button :disabled="!stopLossCount" height="26px" paddingW="6px" @click="cancelOrder(2)">
          <!-- 撤销所有止损单 -->
          {{ lanText.text8 }}
          {{ stopLossCount }}
        </c-button>
...
<script>
import mixin from "@/components/BlockChain-ui-privatization/PC/common-mixin/modules/coTrade/components/stopOrder/stopOrder";
import "@/components/BlockChain-ui-privatization/PC/common-mixin/modules/coTrade/components/stopOrder/stopOrder.styl";
...
.stop-order-mode /deep/.dialog-frame-body {
  height: 550px !important;
}
.stop-order-mode /deep/.dialog-frame {
