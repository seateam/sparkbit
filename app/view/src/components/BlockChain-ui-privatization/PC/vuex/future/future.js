...
export default {
  ...
  actions: {
    // 获取合约列表数据
    getFutorePublicInfo({ commit, dispatch }, payload) {
      let option = {
        method: 'post',
        hostType: 'co',
      }
      if (payload && payload.fullUrl) {
        option.fullUrl = payload.fullUrl;
      } else {
        option.url = this.state.url.futures.publicInfo
      }
      axios(option).then((rs) => {
        if (rs.code === '0') {
