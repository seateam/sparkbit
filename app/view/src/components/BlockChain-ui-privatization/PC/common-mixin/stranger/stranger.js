...
export default {
  ...
  data() {
    return {
      ...
      navTab: [
        {
          name: this.$t('stranger.sell'),
          index: 1,
        },
        {
          name: this.$t('stranger.buy'),
          index: 2,
        },
      ],
      lineHeight: '55',
      marginRight: 32, // 距离右边的距离
      // table 相关
      columns: [
        {
  ...
  methods: {
              ...
              [
                {
                  ...
                  text: `<span>${fixD(obj.price,
                    market.coinList[obj.coin].fiatPrecision[obj.payCoin.toLowerCase()]
                      ...
                      : this.$store.state.baseData.defaultFiatPrecision)}</span> ${obj.payCoin}`,
                  classes: 'price',
                },
              ],
              [
              ...
              [
                {
                  ...
                  iconClass: [''],
                  eventType: 'close',
                  classes: [obj.side === 'SELL' ? 'buy' : 'sell'],
                },
              ],
            ],
          },
    ...
    paymentsList(data) {
      const list = [];
      data.forEach((obj) => {
        list.push(`<li><img src="${obj.icon}"/><span>${obj.title}</span></li>`);
      });
      return list;
    },
    blackClick() {
