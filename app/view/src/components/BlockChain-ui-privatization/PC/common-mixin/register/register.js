...
export default {
  ...
  mounted() {
    // 改，2022.12.16 从路径获取参数
    const { nowType } = this.$route.query;
    if (nowType) {
      this.$nextTick(() => {
        this.setNowType(nowType);
      });
    }
    // 从缓存获取参数
    ...
    if (registerEmail) {
      this.$nextTick(() => {
        // 设置类型，赋值和清缓存
        this.setNowType("email");
        this.userValue = registerEmail;
        ...
        if (scrollPosition > 0) {
          window.scrollTo({
            top: 0,
            behavior: "smooth", // 可选，设置为 "smooth" 时会有平滑滚动效果
          });
        }
      });
    }
  ...
  computed: {
    ...
    tabList() {
      const loginRegistType = +this.loginRegistType;
      const list = [];
      if ([1, 2].includes(loginRegistType)) {
        // 手机号找回
        list.push({ name: this.$t('resetPass.phone'), key: 'phone' });
      }
      if ([1, 3].includes(loginRegistType)) {
        // 邮箱找回
        list.push({ name: this.$t('resetPass.email'), key: 'email' });
      }
      return list;
      // return [
      //   // 手机号找回
  ...
  methods: {
    ...
    submit() {
      ...
      // const passValue = Base64.encode(this.passValue).replace(/=/g, "");
      // const comfirmValue = Base64.encode(this.comfirmValue).replace(/=/g, "");
