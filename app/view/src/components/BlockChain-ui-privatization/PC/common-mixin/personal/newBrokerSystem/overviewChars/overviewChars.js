...
export default {
  data() {
    return {
      charsBar: null,
      chartLine: null,
      volumeYesterday: '--', // 个人昨日交易量
      volumeMonth: '--', // 个人自然月交易量(USDT)
      amountTotal: '--', // 总数额
  ...
  methods: {
    ...
    setBonusData() {
      ...
      this.volumeYesterday = fixD(data.volume_yesterday, this.usdtFix); // 个人昨日交易量
      this.volumeMonth = fixD(data.volume_month, this.usdtFix); // 个人自然月交易量(USDT)
    ...
    setBarData() {
      ...
      this.charsBar.setOption({
        legend: {
          data: [
            this.$t('brokerSystem.charts[0]'),
            this.$t('brokerSystem.charts[1]'),
          ],
        },
        series: [
          {
    ...
    initChartsBar() {
      this.charsBar = window.echarts.init(
        document.getElementById('overviewChars-bar'),
      );
      this.charsBar.setOption({
        tooltip: {
          trigger: 'item',
    ...
    initChartsLine() {
      this.chartLine = window.echarts.init(
        document.getElementById('overviewChars-line'),
      );
      const bg = {
        normal: {
