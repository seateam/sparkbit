  ...
  methods: {
    init() {
      this.$store.dispatch("getUserInfo").then((res) => {
        // 改，2022.12.28 获取裂解参数
        const isVerification = this.$route.query.verification;
        const authLevel = this.authLevel;
        this.$nextTick(() => {
          if (isVerification == 1) {
            if (authLevel === 0) {
              this.btnLink("goID");
            } else if (authLevel === 2 || authLevel === 3) {
              this.btnLink("identity");
            }
          }
        });
      });
    ...
    kycPhoneConfirm() {
      ...
      if (this.nameVerifiedType === "00" && this.country === "+156") {
        ...
        this.$router.push(
          `/personal/idAuth?country=${countryNum}&countryKeyCode=${countryKeyCodeNum}`
        );
        // }
        this.kycLoading = false;
      }
