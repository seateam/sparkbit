...
export default {
  ...
  computed: {
    ...
    symbolType() {
      return this.$t("broker.symbolSelect");
    },
    type() {
      return this.$t("broker.typeSelect");
    },
    tradeTypeText() {
      return this.$t("broker.tradeTypeSelect");
    },
    isLogin() {
    ...
    coinOPtion() {
      ...
      if (this.coinList) {
        ...
        coinList.forEach((item) => {
          if (this.coinList[item].mainChainType !== 2) {
            arr.push({
    ...
    tradeTypeOption() {
      return [
        { value: this.$t("broker.spot"), code: "spot", key: "USDT" },
        { value: this.$t("broker.future"), code: "contract", key: "E_USDT" },
      ];
    },
    tabList() {
      return [
        ...
        return [
          ...
          // },
          {
            title: `${this.$t("broker.fee")}${
              this.tradeTypeValue === "spot"
                ? `(${getCoinShowName(this.coinValue, this.coinList)})`
                : "(USDT)"
            }`,
            width: "150px",
          },
  ...
  methods: {
    ...
    userInfoReady() {
      this.vuserName =
        this.userInfo.mobileNumber !== "" ? this.userInfo.mobileNumber : this.userInfo.email; // 有电话号码显示电话号码,无则显示邮箱
    },
    // 翻页事件
    pagechange(num) {
    ...
    selectOnChange(data, name) {
      ...
      // 当合约/现货筛选变化时，设置对应的查询币种
      if (name === "tradeTypeValue") {
        // 直接使用选项中的key值来设置币种
        this.tradeTypeValue = data.code;
        this.coinValue = data.key;
        this.getData(true);
      }
    },
