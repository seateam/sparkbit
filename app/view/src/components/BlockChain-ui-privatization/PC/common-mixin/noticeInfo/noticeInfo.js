...
export default {
  ...
  data() {
    return {
      imgMap,
      contentId: null, // parseFloat(myStorage.get('ntId')) || null
      sideList: [],
      notieContent: {},
      menuBarHeight: null,
      isshowLoading: true,
    ...
    getContentData() {
      ...
      this.getData({
        ...
        callback: (data) => {
          ...
          this.routes = [
            {
              name: this.$t("message.noticeInfo"),
              path: `/ex/${lan}/noticeInfo`,
            },
            {
              name: this.notieContent.title,
            },
          ];
        },
      });
