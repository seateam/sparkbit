...
export default {
  data() {
    return {
      ...
      // 2023.01.06 修改背景图
      otcReleaseBg: imgMap.otcReleaseBg,
      backgroundImg: `background: url(${imgMap.otcReleaseBg}) no-repeat right bottom`,
      accountList: [], // 资产列表
      symbol: null, // 当前币种
      symbolList: [], // 币种列表
      side: '', // SELL出售 BUY购买
  ...
  methods: {
    init() {
      ...
      this.routes = [
        {
          name: this.$t("fiatdeal.navList")[7],
          path: `/otc/${this.lan}/`,
        },
        {
          name: this.$t("fiatdeal.navList")[6],
        },
      ];
    },
    alertClose() {
    ...
    initSymbol() {
      ...
      this.symbolList.forEach((item) => {
        console.log(item.name, this.otcPublicInfo.defaultCoin)
        if (item.name === this.otcPublicInfo.defaultCoin) {
          this.symbol = item.index;
        }else {
          // 2023.01.06 设置的默认值有问题，先注释掉
          this.symbol = this.symbolList[0].index;
        }
      });
    },
    // 初始化交易的法币
