    ...
    getOtherTypeList() {
      this.axios({
        ...
        if (data.code.toString() === '0') {
          ...
          for (let i = 0; i < cenceList.length; i++) {
            const item = cenceList[i];
            if (this.lan === "zh_CN") {
              list.push({ code: item.key, value: item.key_text });
            } else {
              list.push({ code: item.key, value: cenceListStr[i] });
            }
          }
          this.otherTypeList = list.filter(item => !notShow.includes(item.code));
          if (list.length) {
