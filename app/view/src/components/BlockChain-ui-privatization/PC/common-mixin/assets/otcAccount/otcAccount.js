...
export default {
  ...
  data() {
    return {
      ...
      transferSide: "1", // 划转方向 1 为从现货到法币  2 为法币到现货
  ...
  methods: {
    ...
    // 处理列表数据
    async setData({ totalBalance, totalBalanceSymbol, allCoinMap }) {
      const { coinList, rate } = this.market;
      const FFix =
        (coinList[totalBalanceSymbol] && coinList[totalBalanceSymbol].showPrecision) || 0;
