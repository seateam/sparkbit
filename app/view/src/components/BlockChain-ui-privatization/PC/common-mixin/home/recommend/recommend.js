import { myStorage, colorMap, imgMap } from '@/utils';

export default {
  name: 'recommend',
  data() {
    return {
      imgMap, // 改，2022.12.19 新增图片映射
      // 滚动组件配置
      ops: {
        scrollPanel: {
          speed: 400,
  ...
  computed: {
    ...
    calCoinIcon() {
      return (symbol)=>{
        ...
        coinList = Object.values(coinList)
        if(symbol) {
          const curCoin = coinList.find(item => item.name == symbol) || {};
          return curCoin.icon || ""
        }
        return ""
      }
    ...
    itemWidth() {
      return this.isCoOpen
        ? this.coHeaderSymbol.length * 300 - 20 : this.headerSymbol.length * 310 - 20;
    },
    headerSymbol() {
