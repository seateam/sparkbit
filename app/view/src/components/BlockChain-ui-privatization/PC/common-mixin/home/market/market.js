  ...
  computed: {
    ...
    tableData() {
      ...
      if (this.coAreaData.length) {
        ...
        coArr.forEach((el) => {
          const item = el;
          const datas = el.data.filter(
            (e) => e.isShow || this.listfilter || this.marketCurrent === "USDT"
          );
          if (datas.length) {
            item.data = datas;
            const newItem = item;
      ...
      arr.forEach((el) => {
        ...
        const data = el.data.filter(
          (e) => e.isShow || this.listfilter || this.marketCurrent === "myMarket"
        );
    ...
    columns() {
      return [
        ...
        {
          title: this.isCoOpen
            ? `${this.$t("home.H_volume")}(${this.$t("common.zhang")})`
            : this.$t("home.H_volume"), // 24H成交量
        },
        // 改，2022.11.24 修改标题
        // {
        //   title: this.isCoOpen
        //     ? `${this.$t("home.H_coNumber")}(${this.$t("common.zhang")})`
        //     : this.$t("home.H_turnover"), // 持仓量
        // },
        {
          title: this.$t("header.market"), // 24小时成交额
        },
      ];
    },
    // 全部市场
      ...
      if (this.sortValue.length) {
        return v.sort((a, b) => {
          ...
          switch (this.sortValue) {
            case "roses":
              return (
                (parseFloat(first.data[2][0].sortVal) || 0) -
                (parseFloat(end.data[2][0].sortVal) || 0)
              );
            case "closes":
              return (
                (parseFloat(first.data[1][0].sortVal) || 0) -
                (parseFloat(end.data[1][0].sortVal) || 0)
              );
            default:
              return (
                (parseFloat(first.data[0][1].sortVal) || 0) -
