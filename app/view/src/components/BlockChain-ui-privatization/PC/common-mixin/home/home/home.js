...
export default {
  data() {
    return {
      ...
      tabType: "USD",
      coHeaderSymbol: [],
      klineDataList2: {}, // 改，2022.11.24 冗余一个变量存储列表echart数据
    };
  },
  computed: {
    coinTagLangs() {
  ...
  methods: {
    // 改，2022.11.24 从socket中请求列表中的数据统计数据
    getTableChart() {
      let { symbolList } = this;
    ...
    onmessageWorker() {
      this.worker.onmessage = (event) => {
        ...
        if (data.type === "WEBSOCKET_DATA") {
          ...
          // 改，2022.11.24 新增一个获取WS数据-用于列表 echart 图
          ...
          this.coHeaderSymbol.forEach((item) => {
            ...
            if (this.klineDataList[item.symbol].length) {
              if (Number(lastId) === Number(wsId)) {
                ...
                this.klineDataList[item.symbol].push([timestamp, close]);
              }
            }
          });
          this.$bus.$emit("RECOMMEEND_KLINE_DATA", this.klineDataList);
        ...
        keyarr.forEach((item) => {
          ...
          if (itemData) {
            ...
            let iconSvg = `<span style="font-family: Outfit; font-style: normal; font-weight: 600; font-size: 16px; line-height: 20px; color: var(--color-1);">${
              showName.split("/")[0]
            }</span><span style="font-family: Outfit; font-style: normal; font-weight: 500; font-size: 12px; line-height: 16px; color: #999999;"> /${
              showName.split("/")[1]
            }</span>`;
    ...
    myMarketIcon(symbol) {
      if (this.mySymbolList.indexOf(symbol) === -1) {
        return `<img src="${imgMap.star}" class="icon-24px">`;
      }
    ...
    getLundata() {
      ...
      if (this.$route.query && this.$route.query.preview && this.$route.query.homeBoard) {
        dataObj.preview =
          Number(this.$route.query.preview) > 1 ? 0 : Number(this.$route.query.preview);
          ...
          if (data.errno === "OK") {
            ...
            if (lengthNumber && lengthNumber.length) {
              lengthNumber.forEach((ele) => {
                this.klineDataList[item.symbol].push([ele.timestamp, ele.close]);
              });
            }
            this.$bus.$emit("RECOMMEEND_KLINE_DATA", this.klineDataList);
          }
