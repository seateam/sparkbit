import {
  myStorage, setCookie, imgMap, colorMap,
} from '@/utils';
...
export default {
  ...
  computed: {
    ...
    selectOptions() {
      const arr = [
        { value: this.$t('login.GoogleAuthenticator'), code: 'google' },
      ];
    ...
    thirdList() {
      if (
        this.publicInfo
        && this.publicInfo.thirdInfo
        && this.publicInfo.thirdInfo.length > 0
      ) {
        const arr = [];
  ...
  methods: {
    ...
    goUrl(url) {
      ...
      if (url === '/register' && this.singPassCode && this.singpassState) {
        this.$router.push(
          `${url}?singPassCode=${this.singPassCode}&singPassState=${this.singpassState}`,
        );
      } else {
    ...
    getMySymbol() {
      this.axios({
        ...
        if (data.code === '0') {
          ...
          const sumMySymbolList = Array.from(
            new Set([...storageMySymbol, ...webMySymbol]),
          );
          const diffMySymbolList = storageMySymbol.filter(
            (x) => !webMySymbol.includes(x),
          );
          // 同步本地自选币对和服务端币对
