...
export default {
  ...
  data() {
    return {
      ...
      isInitWidth: false, // 获取列表后，只初始化一次宽度
    };
  },
  watch: {
    positionList(val) {
  ...
  methods: {
    ...
    setCellWidth() {
      ...
      tdFefsKeys.forEach((item) => {
        if (item.indexOf('td') !== -1) {
          if (this.$refs[item][0]) {
            cellWidth.push(`${this.$refs[item][0].offsetWidth}px`);
          }
        }
      });
    ...
    closeOrder(type, data) {
      ...
      console.log("volume, canCloseVolume", volume, canCloseVolume)
      if (volume > Number(canCloseVolume)) {
        // 超出可平数量
    ...
    init() {
      ...
      // 改，2022.11.22 获取 盘口 和 成交点击过来的价格
      // 获取 盘口 和 成交点击过来的价格
      this.$bus.$on('HANDEL_PRICE', (data) => {
        // 改， 2022.12.05 修改tableDataList数据结构
        let tableDataList = Object.values(this.tableDataList || {})
        if(tableDataList && tableDataList.length) {
          for (let i = 0; i < tableDataList.length; i++) {
            ...
            // 触发原input事件
            this.priceChange(event, tableDataList[i]);
          }
        }
      });
    },
