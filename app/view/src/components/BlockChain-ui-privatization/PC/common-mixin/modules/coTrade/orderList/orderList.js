...
export default {
  ...
  computed: {
    ...
    tabTypeItem() {
      return [
        // 持仓
        this.isLogin ? `${this.lanText.tabType1}` : this.lanText.tabType1, //  (${this.positionListNumber})
        this.isLogin ? `${this.lanText.tabType2}` : this.lanText.tabType2, // '普通委托' (${this.orderCount})
        this.isLogin ? `${this.lanText.tabType3}` : this.lanText.tabType3, // '条件委托' (${this.triggerOrderCount})
  ...
  methods: {
    ...
    // 只显示当前合约开关
    switchChange(e) {
      console.log(e, this.isShowCurPosition);
