import { fixD, division, getDigit, cut, thousandsComma, myStorage, nul } from "@/utils";
...
export default {
  ...
  data() {
    return {
      // 按钮颜色class类型
      showClassType: "depositDialogShow",
      // 交易类型 1开仓 2 平仓
      transactionType: 1,
      // 1 限价 2 市价 3 条件
      currentCategory: 1,
    ...
    switchTradeTypeList() {
      ...
      return [
        ...
        },
        {
          ...
          classes: this.transactionType === 2 ? "u-4-bg b-1-cl" : "a-8-bg b-2-cl",
        },
      ];
    },
    // 限价单\市价单\条件单
    categoryList() {
      // 改，2022.08.28，a-5-bg -> u-8-bg
      return [
        {
          id: 1,
          text: this.lanText.text22, // '限价单',
          classes: this.currentCategory === 1 ? "u-8-bg b-1-cl" : "a-8-bg b-2-cl",
        },
        {
          id: 2,
          text: this.lanText.text23, // '市价单',
          classes: this.currentCategory === 2 ? "u-8-bg b-1-cl" : "a-8-bg b-2-cl",
        },
        {
          id: 3,
          text: this.lanText.text24, // '条件单',
          classes: this.currentCategory === 3 ? "u-8-bg b-1-cl" : "a-8-bg b-2-cl",
        },
      ];
    },
    // 可买 可卖标题文案
    ...
    inputPrice() {
      ...
      if (this.currentCategory === 3) {
        if(this.isMarket){
          ...
          console.log("this.formData_2.value", this.formData_2.value);
  ...
  methods: {
    ...
    countPercentageVlaue(type) {
      ...
      if (this.isOpenAndIsMarket) {
        ...
        // 根据杠杆限制算出可开
        const val2 =
          type === "BUY" ? nul(percentageVlaue, this.V1Buy) : nul(percentageVlaue, this.V1Sell);
    ...
    formNumberVerify() {
      ...
      if (this.inputPrice) {
        const range = Math.abs(
          division(cut(this.activeNewPrice, this.inputPrice), this.activeNewPrice)
        );
      ...
      if (vol) {
        ...
        if (this.isOpenAndIsMarket) {
          ...
          if (vol > Number(canMaxNumberZhang)) {
            // 超出最大下单量，最多还可以下单
            this.formData_3.errorText = this.lan === 'en_US' ? this.lanText.text56 : `${this.lanText.text56} ${canMaxNumber} ${this.volUnit}`;
            flag = false;
            return false;
          }
          ...
          if (this.zhiJianCang || this.transactionType === 2) {
            // 可平数量
            const Kp =
              sideType === "SELL" ? Number(this.maxCanSellNumber) : Number(this.maxCanBuyNumber);
            // 如果当前单位是标的货币
            if (this.coUnitType === 1) {
    ...
    submitForm(sideType) {
      ...
      if (this.userConfig && this.userConfig.pcSecondConfirm) {
        ...
        this.confirmData = {
          ...
          // 价格
          price: this.inputPrice ? `${this.inputPrice} ${this.priceUnit}` : this.$t("trade.marketPrice"),
          // 数量
          vol,
    ...
    setHeight() {
      let setTimer = setTimeout(() => {
        const dom1 = document.getElementById("layoutBlockTrade")
        const dom2 = document.getElementById("coTradeBox")
        const layoutHeight = dom1 ? dom1.clientHeight : 0;
        const coTradeBoxHeight = dom2 ? dom2.clientHeight : 0;
