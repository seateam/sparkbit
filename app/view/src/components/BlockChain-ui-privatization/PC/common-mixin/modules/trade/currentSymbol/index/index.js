...
export default {
  ...
  computed: {
    calCoinName() {
      const symbolCurrent = this.symbolCurrent.split("/");
      return symbolCurrent[0];
    },
    // 2023.01.05 当前币种icon
    curCoinIcon() {
      let symbolCurrent = this.symbolCurrent || ""
      let coinName = symbolCurrent.split("/")[0] || ""
      if (coinName && this.coinList) {
        return this.coinList[coinName].icon || ""
      }
    },
    coinTagLangs() {
    ...
    symbolsData() {
      ...
      if (this.dataList[this.symbolCurrent]) {
        data = this.dataList[this.symbolCurrent];
      }
      this.$bus.$emit("LAST_PRICE", data);
      return data;
    },
  },
  methods: {
