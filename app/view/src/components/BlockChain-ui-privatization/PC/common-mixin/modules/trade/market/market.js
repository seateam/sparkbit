...
export default {
  ...
  computed: {
    ...
    marketList() {
      if (this.marketData) {
        if (this.moduleType === "ex") {
          return this.marketData.marketSort;
        }
        if (this.moduleType === "lever") {
          const keysArr = this.marketData.marketSort;
          const marketDataObj = this.marketData.market;
          const newSort = [];
    ...
    // 全部币种列表
    coinList() {
      if (this.$store.state.baseData && this.marketData) {
        return this.marketData.coinList;
      }
      return null;
    },
  },
  ...
  methods: {
    ...
    setData() {
      const dataList = [];
      const keyarr = this.symbolList ? Object.keys(this.symbolList) : [];
      const coinList = this.coinList || {};
      this.maincoinFlag = false;
      this.newcoinFlag = false;
      this.seecoinFlag = false;
      ...
      keyarr.forEach((item) => {
        ...
        if (this.marketDataList[item]) {
          if (isShow || this.listfilterVal || this.marketCurrent === "myMarket") {
            ...
            if (newcoinFlag === 3 && coinList[symbol] && !coinList[symbol].isOvercharge) {
