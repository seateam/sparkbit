...
export default {
  ...
  data() {
    return {
      formData_1: {
        ...
        units: myStorage.get('leverSymbolName')
          ? myStorage.get('leverSymbolName').split('/')[1]
          : 'USDT',
        subText: '',
        errorText: '',
        isError: false,
      ...
      formData_2: {
        ...
        units: myStorage.get('leverSymbolName')
          ? myStorage.get('leverSymbolName').split('/')[0]
          : 'USDT',
        subText: '',
        errorText: '',
        isError: false,
  ...
  computed: {
    ...
    buttosContent() {
      return {
        buyButton: {
          text: this.isLogin
            ? `${this.$t('lever.buy')} ${getCoinShowName(
              this.symbolUnit.symbol,
              this.coinList,
            )}`
            : this.$t('trade.loginReg4'), // '登录/注册',
            // 改，2022.09.24，颜色 u-1-bg -> u-8-bg
          class: this.isLogin
            ? 'u-8-bg buyBtn u-16-cl'
            : 'u-8-bg buyBtn u-16-cl',
        },
        sellButton: {
          text: this.isLogin
            ? `${this.$t('lever.sell')} ${getCoinShowName(
              this.symbolUnit.symbol,
              this.coinList,
            )}`
            : this.$t('trade.loginReg4'), // '登录/注册',
          class: this.isLogin
            ? 'u-4-bg sellBtn u-16-cl'
            : 'u-8-bg sellBtn u-16-cl',
        },
      };
    },
    // 账户资产
    ...
    currenTaccount() {
      if (this.financeData) {
        return {
          symbolAccoubt: fixD(
            this.financeData.baseNormalBalance,
            this.valuePrecision(this.financeData.baseCoin)
          ),
          unitsAccoubt: fixD(
            this.financeData.quoteNormalBalance,
            this.valuePrecision(this.financeData.quoteCoin)
          ),
        };
      }
      return {
  ...
  methods: {
    ...
    onChaneForm(data) {
      ...
      // 不得超过可用余额
      if (data.name === "formData_2" && this.transactionType === 1 && (data.value) * this.formData_1.value > UA) {
    ...
    setPerNumber(type, num) {
      if (!this.isLogin) {
        ...
        if (!this.formData_1.value && this.transactionType === 1) {
          ...
          this.perBuy = num;
          const data = (this.currenTaccount.unitsAccoubt / this.formData_1.value)
            * (num / 100);
        ...
        if (formKey.length) {
          formKey.forEach((item) => {
            ...
            if (!value) {
              ...
              this.$bus.$emit('tip', { text: `${this.$t('trade.pleaseInput')}${this[item].title}`, type: 'error' });
              // this[item].isError = true;
              fal = false;
            }
            if (parseFloat(value) <= 0) {
              // this[item].errorText = `${this[item].title}${this.$t('trade.inputError')}`; // 输入有误
              this.$bus.$emit('tip', { text: `${this[item].title}${this.$t('trade.inputError')}`, type: 'error' });
              // this[item].isError = true;
              fal = false;
            }
          });
      ...
      if (this.isLogin && this.symbolCurrent) {
        ...
        this.axios({
          ...
          if (data.code === '0') {
            ...
            if (this.symbolAll) {
              quotePrecision = this.symbolAll[ndata.name]
                ? this.symbolAll[ndata.name].price
                : 8;
            }
            ...
            // 2023.6.12 获取资产数据
            this.$emit("leverFinance", ndata);
            // 可用
            this.financeData.baseNormalBalance = fixD(
              ndata.baseNormalBalance,
              basePrecision,
            );
            this.financeData.quoteNormalBalance = fixD(
              ndata.quoteNormalBalance,
              quotePrecision,
            );
            // 可借
            ...
            // 已借
            this.financeData.baseBorrowBalance = fixD(
              ndata.baseBorrowBalance,
              basePrecision,
            );
            this.financeData.quoteBorrowBalance = fixD(
              ndata.quoteBorrowBalance,
              8,
            );
            // 总额度
            ...
            // 爆仓价
            this.financeData.burstPrice = fixD(
              ndata.burstPrice,
              quotePrecision,
            );
            this.$bus.$emit("financeData", this.financeData);
          }
        });
      }
    },
