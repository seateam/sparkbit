...
export default {
  ...
  data() {
    return {
      ...
      // 卖盘 高度 - 弃用，使用 plateHeight 代替
      sellHeight: 306,
      // 买盘 高度 - 弃用，使用 plateHeight 代替
      buyHeight: 306,
      // 显示条数 - 弃用，使用 showTypeNum 代替
      sellLineNumber: 14,
      buyLineNumber: 14,
      // 当前价格
      activePrice: null,
      priceClsss: null,
      showLineNum: 10, // 展示多少行数据
      showTypeNum: 1, // 展示类型
      plateHeight: 200, // 盘口高度 px
    };
  },
  computed: {
    templateLayoutType() {
        ...
        if (dataTypeKey.length) {
          dataTypeKey.forEach((item) => {
            ...
            objKeys.forEach((itemKey) => {
              ...
              // 展示数量
              if (dataArr.length < this.showLineNum) {
                // 获取最大的数量
  ...
  watch: {
    ...
    showTypeNum(val) {
      switch (val) {
        case 1:
          this.showLineNum = 10;
          this.plateHeight = 200;
          break;
        case 2:
          this.showLineNum = 28;
          this.plateHeight = 403;
          break;
        case 3:
          this.showLineNum = 28;
          this.plateHeight = 403;
