import { fixD, nul, colorMap, imgMap, getCoinShowName } from "@/utils";
...
export default {
      ...
      if (otcTicker) {
        if (this.publicInfo) {
          if (this.otcPublicInfo) {
            this.lastPrice = this.dealOtcTicke(otcTicker.data.lastPrice);
            this.avgPrice = this.dealOtcTicke(otcTicker.data.avgPrice);
    ...
    publicInfo(publicInfo) {
      if (publicInfo) {
        if (this.otcPublicInfo) {
          ...
          // 头部数据
          if (this.otcTicker) {
            this.lastPrice = this.dealOtcTicke(this.otcTicker.data.lastPrice);
            this.avgPrice = this.dealOtcTicke(this.otcTicker.data.avgPrice);
  ...
  methods: {
    ...
    operation(type, id) {
      ...
      switch (type) {
        case "icon":
          ...
          break;
        default:
          if (this.side === "BUY" && !this.isPaymentMatch(id[2])) {
            // 要出售时并无支付方式时
    ...
    isPaymentMatch(payments) {
      ...
      // 检查支付方式是否匹配
      return payments.some((obj) =>
        this.paymentList.some((payment) => obj.key === payment.payment)
      );
    },
    pagechange(data) {
      this.page = data;
          ...
          list.push({
            ...
            data: [
                  ...
                  iconSvg: `<div class="merchants">
                    ...
                    ${
                      obj.imageUrl
                        ...
                        : `<span>${obj.otcNickName.substring(0, 1)}</span>`
                    }
                    </div>
                    <div class="merchants-right">
                    <p>${obj.otcNickName}</p>
                    <p class="b-2-cl"><i class="i a-5-bd ${
                      obj.loginStatus === 1 ? "a-18-bg" : "a-1-bg"
                    }"></i>${this.$t("fiatdeal.center.transactions")} ${
                    obj.completeOrders
                  } | ${this.$t("fiatdeal.center.credit")} ${nul(obj.creditGrade, 100)}%</p>
                    </div>
                    </div>`,
                },
              ],
              ...
              [
                {
                  type: "html",
                  text: `<span>${fixD(obj.price, this.priceFix)}</span> ${obj.payCoin}`,
                  classes: "price",
                },
              ],
              [
              ...
              [
                {
                  ...
                  text:
                    this.side === "SELL"
                      ? this.$t("fiatdeal.center.buy")
                      : this.$t("fiatdeal.center.sell"),
                  iconClass: [""],
                  eventType: "check",
                  classes: [this.side === "SELL" ? "sell" : "buy"],
                },
              ],
            ],
          });
    ...
    paymentsList(data) {
      const list = [];
      data.forEach((obj) => {
        list.push(`<li><img src=${obj.icon} /> ${obj.title}</li>`);
      });
      return list;
    },
      ...
      list.forEach((obj) => {
        if (Number(obj.otcOpen) === 1) {
          ...
          this.navTab.push({
            ...
            index: count,
            coinName: obj.name,
            // 为c-select组件添加必需的code和value属性
            code: count,
            value: getCoinShowName(obj.name, this.market.coinList),
          });
          if (obj.name === symbol) {
    ...
    dataPayments(data) {
      ...
      data.forEach((obj) => List.push({ value: obj.title, code: obj.key }));
      this.list3 = List;
    ...
    dataCountry(data) {
      const List = [];
      List.push({ value: this.$t("fiatdeal.center.all"), code: "all" });
      data.forEach((obj) => List.push({ value: obj.title, code: obj.numberCode }));
      this.list1 = List;
      this.value1 = "all";
    },
  },
