...
export default {
  ...
  watch: {
    ...
    checkValue(value) {
      ...
      if (this.checkValue && !this.checkFlag) {
        this.checkErrorText = this.$t("login.phoneCodeError");
      } else {
        this.checkErrorText = "";
      }
    },
    emailCheckValue(value) {
      ...
      if (this.emailCheckValue && !this.checkFlagEamil) {
        this.checkErrorText = this.$t("login.emailCodeError");
      } else {
        this.checkErrorText = "";
      }
    },
    googleVlaue(value) {
      ...
      if (this.googleVlaue && !this.googleFlag) {
        this.googleErrorText = this.$t("login.googleCodeError");
      } else {
        this.googleErrorText = "";
      }
    },
  },
  computed: {
  ...
  methods: {
    ...
    setStatusText(data) {
      ...
      if (nowTime < data.startTime && data.status !== 2) {
        return this.$t("innov.status0");
      }
      switch (data.status) {
        case 2:
    ...
    goTrade() {
      const { pageData } = this;
      if (pageData.holdCoin === "USDT") {
        this.$router.push("/trade/BTC_USDT");
        return;
      }
      this.$router.push(`/trade/${pageData.holdCoin}_USDT`);
    },
  },
};
