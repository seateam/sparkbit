...
export default {
  ...
  methods: {
    init() {
      ...
      // this.getUserGainList(this.currentType);
      this.routes = [
        {
        ...
        {
          name: this.$t("freeStaking.home.myholdings"),
        },
      ];
    },
    getMonth(n) {
    ...
    // 获取明细列表
    getUserGainList(currentType) {
      this.axios({
        url: "increment/project/incrementActDetail",
        headers: {},
        params: {
          page: currentPage,
          pageSize: display,
    ...
    initPosData(data) {
      ...
      const posData = data.map((item, index) => {
        item.activeStatus = item.activeStatus > 7 ? 7 : item.activeStatus;
        const lockData = [
          item.baseCoin,
          item.gainStartTime,
          item.userCurrentAmount,
          `${item.gainRate}%`,
          item.userGainAmount,
          // [
          //   {
          //     text: item.userGainAmount,
          //   },
          //   {
          //     type: 'subTable',
          //   },
          // ],
          this.countStatusText(item.activeStatus),
        ];
        const unLockData = [
          item.baseCoin,
          item.gainEndTime,
        ...
        const subPosDataItem =
          !this.currentType && Array.isArray(item.userGainList) ? item.userGainList : [];
    ...
    countStatusText(status) {
      const statusText = {
        ...
        7: this.$t("freeStaking.home.status[7]"),
      };
