import { fixD, colorMap, imgMap, myStorage } from "@/utils";
...
export default {
  ...
  computed: {
    ...
    totalGainAmount() {
      ...
      return !this.isLogin || !this.projectDetail
        ? "- - -"
        : fixD(totalGainAmount, this.currentSymbol);
    },
    totalUserGainAmount() {
      const { totalUserGainAmount } = this.projectDetail;
      return !this.isLogin || !this.projectDetail
        ? "- - -"
        : fixD(totalUserGainAmount, this.currentSymbol);
    },
    raiseAmount() {
    ...
    lockMinNumText() {
      ...
      return `${text} (${text1} ${buyAmountMin}${shortName || "-"}, ${text2} ${buyAmountMax}${
        shortName || "-"
      } )`;
    },
    symbolAll() {
    ...
    userFeatureIncome() {
      ...
      const { totalAmount, gainRate, lockDay, currencyExchangeRate } = this.projectDetail;
    ...
    projectStatus() {
      ...
      const result = projectStatus.map((item, index) => {
        const datetime = this.countStateTime(index);
        const times = new Date(datetime).getTime();
        // returnType //本金释放状态，0：未释放，1：释放中，2：释放完成
        return {
          ...item,
          datetime,
          active: item.returnType === 2 || nowTime > times,
        };
      });
      return result;
    },
    ...
    userIncomeTitle() {
      ...
      // 新语义：已申购 lockDay 日理财预计收益（使用前后缀键）
      const prefix = this.$t("freeStaking.detail.userIncomeTitlePrefix");
      const suffix = this.$t("freeStaking.detail.userIncomeTitleSuffix");
      return `${prefix}${lockDay}${suffix}`;
      // // 回退到旧逻辑：{lockDay} {day}{text}
  ...
  methods: {
    ...
    countStateTime(index) {
      ...
      switch (index) {
        case 0:
          time = this.projectDetail.startApplyTime;
          break;
        case 1:
          time = this.projectDetail.endApplyTime;
          break;
        case 2:
          time = this.projectDetail.gainStartTime;
          break;
        case 3:
          time = this.projectDetail.gainEndTime;
          break;
        default:
          break;
      }
    ...
    getProjectDetail() {
      this.axios({
        ...
        if (data.code === "0") {
          ...
          // 2023年7月4日
          projectDetail.shortName = projectDetail.baseCoin;
          projectDetail.balance = projectDetail.userNormalAmount;
          // 从缓存获取数据  && listDetail.id === projectDetail
          const listDetail = myStorage.get("takeActivityDetail");
          const { id } = this.$route.params;
          if (listDetail && listDetail.id == id) {
            projectDetail.progress = listDetail.progress;
            projectDetail.raiseAmount = listDetail.raiseAmount;
          }
          ...
          this.routes = [
            {
              name: this.$t("freeStaking.home.staking"),
              path: `/ex/${lan}/freeStaking`,
            },
            {
    ...
    submitUserPos() {
      const { buyAmountMin, buyAmountMax, balance, shortName, totalAmount } = this.projectDetail;
      let errText = "";
    ...
    dialogConfirm() {
      ...
      this.axios({
        ...
        if (data.code.toString() === "0") {
          this.$bus.$emit("tip", {
            text: this.$t("freeStaking.detail.buySuccess"),
            type: "success",
          });
          this.dialogFlag = false;
          this.initData();
