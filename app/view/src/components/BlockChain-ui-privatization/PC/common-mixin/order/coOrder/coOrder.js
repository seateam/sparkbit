    ...
    columns() {
      ...
      if (this.nowType === 1) {
        list = [
          ...
          { title: this.$t('futures.order.time'), width: '8%' }, // 时间
          { title: this.$t('futures.order.orderType'), width: '8%' }, // 订单类型
          { title: this.$t('futures.order.side'), width: '8%' }, // 方向
          { title: this.$t('futures.order.intrustPrice'), width: '10%' }, // 委托价格
          { title: this.$t('futures.order.intrustVolume'), width: '12%' }, // 委托数量
          { title: this.$t('futures.order.dealVolume'), width: '12%' }, // 成交数量
      ...
      if (this.nowType === 2) {
        list = [
          ...
          { title: this.$t('futures.order.intrustPrice'), width: '8%' }, // 委托价格
          { title: this.$t('futures.order.intrustVolumeOrNum'), width: '14%' }, // 委托数量/价值
      ...
      if (this.nowType === 3) {
        list = [
          { title: this.$t('futures.order.contract'), width: '8%', align: 'left' }, // 合约
          { title: this.$t('futures.order.time'), width: '10%' }, // 时间
          { title: this.$t('futures.order.orderType'), width: '9%' }, // 订单类型(张)
          { title: this.$t('futures.order.side'), width: '9%' }, // 方向
          { title: this.$t('futures.order.intrustPrice'), width: '14%' }, // 委托价格
          { title: this.$t('futures.order.intrustVolumeOrNum'), width: '14%' }, // 委托数量/价值
          { title: this.$t('futures.order.dealVolume'), width: '12%' }, // 成交数量
      ...
      if (this.nowType === 4) {
        list = [
          { title: this.$t('futures.order.contract'), width: '10%', align: 'left' }, // 合约
          { title: this.$t('futures.order.side'), width: '5%' }, // 方向
          { title: this.$t('futures.order.triggerPrice'), width: '10%' }, // 触发价格
          { title: this.$t('futures.order.intrustPrice'), width: '10%' }, // 委托价格
          { title: this.$t('futures.order.intrustVolumeOrNum'), width: '13%' }, // 委托数量/价值
          { title: this.$t('futures.order.onlyCut'), width: '10%' }, // 只减仓
