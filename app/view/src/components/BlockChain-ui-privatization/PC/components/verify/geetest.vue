...
<template>
  <section class="common-geetest" :style="contentStyle">
    ...
    >
      ...
      <!-- 选中时横线 -->
      <!-- 改，2022.09.28，注释 -->
      <!-- <span class="common-geetest-activeLine" :class="activeLineClass" :style="activeLineStyle">
      </span> -->
    </div>
    <!-- 错误文案占位 24px -->
...
export default {
  ...
  computed: {
    ...
    activeLineStyle() {
      ...
      if (this.nowState || this.isHover || (this.errorHave && this.errorFlag && this.isFocus)) {
        border = "1px solid #226afd";
      }
  ...
  methods: {
    ...
    setStyle() {
      const { colorMap } = this;
      const radar = document.getElementsByClassName("geetest_radar_tip_content");
      const reset = document.getElementsByClassName("geetest_reset_tip_content");
      const geetestRadarBtn = document.getElementsByClassName("geetest_radar_btn");
      const geetestSuccessBtn = document.getElementsByClassName("geetest_success_btn");
      ...
      if (colorMap) {
        ...
        // 改，2022.09.28，改颜色，colorMap[this.geetestBg] -> #ffffff
        if (geetestRadarBtn && geetestRadarBtn.length) {
          geetestRadarBtn[0].style.background = "#ffffff";
        }
        if (geetestSuccessBtn && geetestSuccessBtn.length) {
          geetestSuccessBtn[0].style.background = "#ffffff";
        }
      }
    },
    // 构件极验
