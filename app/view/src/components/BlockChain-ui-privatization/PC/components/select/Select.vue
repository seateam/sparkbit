...
<template>
  ...
  >
    ...
    <!-- 改，2022.12.27，修改圆角 2 -> 8 -->
    <div
      class="input_line_content a-2-bd"
      @click="toggleMenu"
    ...
    >
      ...
      <input
        ...
        @mouseenter="handMouseenter"
        @mouseout="handMouseleave"
        style="color: var(--color-text-primary) !important; font-size: 14px"
        :style="customInputStyle"
      />
      <!-- 三角ICON -->
      <!-- 改，2022.09.28，删除b-1-bd -->
    ...
    <transition name="slideInDown">
      <div v-show="visible" class="select-options-box a-5-bg b-1-cl">
        <div v-show="selectOption.length" class="select-option-list" :style="setBoxHeight">
          <vue-scroll>
            <!-- 单行类型 -->
            <ul v-if="type === 'info'">
              <li
                ...
                :class="{ actived: value === item.code || index === overInx }"
                :key="index"
                :style="optionStyle"
                ref="selectLi"
        ...
        <div class="not_option" v-show="!selectOption.length && !loading">
          <!-- 暂无数据 -->
          {{ $t("components.select.noData") }}
        </div>
        <div class="not_option" v-show="loading">
          <!-- 加载中 -->
...
export default {
  ...
  computed: {
    ...
    stylees() {
      ...
      // 改，2022.12.22 修改无语BUG，高度赋值给了宽度（styles.width = this.height），
  ...
  watch: {
    ...
    options: {
      handler(newOptions) {
        // 当options变化且有value时，重新设置显示值
        if (this.value && newOptions && newOptions.length > 0) {
          this.values = this.getInitialValue(this.value);
        }
      },
      immediate: false,
    ...
    // 修复多语言切换后promptText消失问题 - 监听promptText变化
    promptText: {
      handler(newVal, oldVal) {
        if (newVal !== oldVal) {
          // 强制触发重新渲染，确保CSS状态正确更新
          this.$nextTick(() => {
            this.$forceUpdate();
          });
        }
