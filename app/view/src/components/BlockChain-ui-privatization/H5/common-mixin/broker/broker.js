...
export default {
  ...
  data() {
    return {
      ...
      symbolType: this.$t("broker.symbolSelect"),
      type: this.$t("broker.typeSelect"),
    };
  },
  watch: {
    coinOPtion(value) {
    ...
    columns() {
      if (this.tableType === "agent_data") {
        return [
          ...
          {
            title: `${this.$t("broker.amount")}(${getCoinShowName(
              this.allBonusCoin,
              this.coinList
            )})`,
            width: "150px",
    ...
    getData(search) {
      ...
      this.axios({
        ...
        if (data.code === "0") {
          ...
          if (
            Math.ceil(parseFloat(data.data.count) / parseFloat(this.pagination.pageSize)) >
