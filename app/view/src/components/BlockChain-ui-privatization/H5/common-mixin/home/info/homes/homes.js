...
import { myStorage, imgMap, colorMap } from "@/utils";
  ...
  methods: {
    ...
    myMarketIcon(symbol) {
      if (this.mySymbolList.indexOf(symbol) === -1) {
        return `<img src="${imgMap.star}" class="icon-20px">`;
      }
      return `<img src="${imgMap.stared}" class="icon-20px">`;
    },
    // 获取 首页数据(公告、轮播图)
    getLundata() {
      this.axios({
        ...
        if (data.code === "0") {
          ...
          if (homeEditHtml && homeEditHtml.length) {
            ...
            if (homeEditHtml.indexOf("<script>") > -1) {
              ...
              scriptHtml = scriptHtml.replace(/<\/script>/gi, "");
    ...
    itemRoseClass(rose) {
      let bgClass = null;
      // 改，2022.10.08，u-3-bg -> u-3-bg-new，u-6-bg -> u-6-bg-new，u-1-cl -> f-1-cl-new
      if (rose.class === "u-1-cl") {
        bgClass = "rose-label u-3-bg-new f-1-cl-new";
      } else if (rose.class === "u-4-cl") {
        bgClass = "rose-label u-6-bg-new f-1-cl-new";
      }
