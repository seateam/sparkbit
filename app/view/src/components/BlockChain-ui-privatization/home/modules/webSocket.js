(() => {
  const { myStorage, setDefaultMarket, getCoinShowName, getScript } = window.BlockChainUtils;
  ...
  const init = () => {
    ...
    const setMarketData = (coinList, symbolAll, initData) => {
      ...
      emitter.emit(
        "MARKET-DATA",
        marketDataList.sort((a, b) => a.sort - b.sort),
        initData
      );
    };
            ...
            if (symbol === symbolType) {
              if (WsData.event_rep === "rep") {
                ...
                if (klineDataList[key].length) {
                  ...
                  klineDataList[key].push([kData.id, kData.close]);
                }
              }
            }
          });
