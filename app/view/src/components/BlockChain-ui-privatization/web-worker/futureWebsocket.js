      ...
      if (data.tick && data.channel.indexOf("depth_step") !== -1) {
        ...
        if (data.channel.indexOf(depthClasses) !== -1) {
          ...
          if (data.tick.side) {
            ...
            if (
              tickData[symbolType]
              && tickData[symbolType][tick.side][tick.price]
              && tickData[symbolType][tick.side][tick.price][1] > tick.volume
            ) {
              diff = 2;
            }
            ...
            if (data.ts - depthTime.socketTime >= depthTime.contrastTime) {
              ...
              sendMessage("WEBSOCKET_DATA", "DEPTH_DATA", tickData[symbolType]);
