...
importScripts(`${webworkerMap.baseUrl}static/js/pako.min.js`);
// 公共方法
importScripts(`${webworkerMap.baseUrl}static/web-worker/utils/${webworkerMap['common-method']}`);

// 24 小时行情数据  setMarketData()
importScripts(`${webworkerMap.baseUrl}static/web-worker/${webworkerMap.marketData}`);
// 盘口深度数据处理 setDepthData()
importScripts(`${webworkerMap.baseUrl}static/web-worker/${webworkerMap.depthData}`);
// 深度图数据处理 setEchartsData()
importScripts(`${webworkerMap.baseUrl}static/web-worker/${webworkerMap.echartsData}`);
// 实时成交数据处理 setTradeData()
importScripts(`${webworkerMap.baseUrl}static/web-worker/${webworkerMap.tradeData}`);
...
const marketReviewCoSend = function () {
  if (MywebSocket) {
    let arr = [];
    const args = [];
    // USD:币本位合约
    // USDT:合约
    arr = symbolAll.filter(item => item.quote_coin === 'USD' || 'USDT');
    symbolAll = [...arr];
    // 合约ID  instrument_id
    arr.forEach((ele) => {
      args.push(`Ticker:${ele.instrument_id}`);
    });
    MywebSocket.send(JSON.stringify({
      action: "subscribe",
      args,
...
const webSocketEvent = function () {
  ...
  MywebSocket.onmessage = function (event) {
    ...
    // 如果是USDT合约请求或者币本位请求
    if (currentSymbol === 'USD' || currentSymbol === 'USDT') {
      const data = JSON.parse(event.data);
      // 重置心跳检测
      heartCheckTop();
      if (data.pong) {
        MywebSocket.send(JSON.stringify({ action: 'ping' }));
      } else {
        // 4是kline数据；1，2是tiker数据；
        if (data.action !== 4) {
          if (data.action === 1) {
            const obj = data.data;
            afterData.push(obj);
          ...
          if (data.action === 2) {
            afterData.forEach((item) => {
              if (item.instrument_id === data.data.instrument_id) {
                const obj = Object.keys(data.data);
                obj.forEach((ele) => {
                  item[ele] = data.data[ele];
                });
              }
            });
          }
          sendMessage(
            'WEBSOCKET_DATA',
            'MARKET_DATA',
            setMarketDataCo(symbolAll, afterData, rate, lan),
          );
        } else {
...
self.addEventListener('message', (event) => {
  ...
  // 监听 创建 CoWS
  if (data.type === 'CREAT_CO_WEBSOCKET') {
    if (!MywebSocket) {
      webSocketUrl = data.data.wsUrl;
      lan = data.data.lan;
      rate = data.data.rate;
  ...
  // 发送CO kline send
  if (data.type === 'WEBSOCKET_KLINE_CO_SEND') {
    if (MywebSocket.readyState === 1) {
  ...
  // 发送 Cosend
  if (data.type === 'WEBSOCKET_CO_SEND') {
    if (MywebSocket.readyState === 1) {
      webSocketSend(data.data);
    }
  }
