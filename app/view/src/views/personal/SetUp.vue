<template>
  <div class="set-up">
    ...
    <p class="user-management-child-header">
      {{
        paymentList.length === 0
          ? this.$t("personal.setUp.title1")
          : this.$t("personal.setUp.title2")
      }}
    </p>
    <div class="center clearfix">
      <p class="center-title b-7-cl" v-if="cardFlag && excheifFlag">
        <svg class="icon icon-16" aria-hidden="true">
          <use xlink:href="#icon-c_2"></use></svg
        >{{ $t("personal.setUp.hint") }}
      </p>
      <c-select
        v-show="set === 0"
      ...
      />
      <Payment1 v-if="payment === 1" @callBack="callBack" :isModify="set" :payments="payments" />
      <Payment2 v-if="payment === 2" :isModify="set" @callBack="callBack" :payments="payments2" />
      <Payment3 v-if="payment === 3" :isModify="set" @callBack="callBack" :payments="payments3" />
      <Payment4 v-if="payment === 4" :isModify="set" @callBack="callBack" :payments="payments4" />
      <c-inputLine
        v-show="smsCode"
        maxLength="6"
        name="smsCode"
...
<script>
import mixin from "@/components/BlockChain-ui-privatization/PC/common-mixin/personal/SetUp/SetUp";
import "@/components/BlockChain-ui-privatization/PC/common-mixin/personal/SetUp/SetUp.styl";
