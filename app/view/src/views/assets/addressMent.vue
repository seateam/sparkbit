<template>
  <section class="page-addressMent">
    ...
    <div
      class="addressBox clearfix a-7-bd"
      style="display: flex; align-items: center; padding: 44px"
    >
      <div class="addressBox-left" :class="{ autoW: isHavePage }">
        <!-- 添加地址 -- 币种 -->
        <div class="addressBox-select">
          <c-select
            :promptText="$t('assets.addressMent.coin')"
            :errorHave="false"
            :value="symbolValue"
            :options="selectList"
            :filterable="true"
        ...
        <!-- 添加地址 -- 链名称 -->
        <div class="addressBox-select" v-if="haveBranch" style="width: 250px">
          <c-select
            :promptText="$t('assets.branchName')"
            :errorHave="false"
            :value="activeBranch"
            :options="branchArr"
            :filterable="true"
          ...
          >
            <span slot="promptExtend" v-if="branchTip">
              <div class="mainChain-title">
      ...
      <div class="addressBox-right">
        ...
        <c-button
          ...
          @click="addressBtnClick"
          >{{ $t("assets.addressMent.add") }}</c-button
        >
      </div>
    </div>
    <header class="addressMent-header b-1-cl a-3-bd">
    ...
    <div class="addressMent-table m-table">
      <c-table
    ...
    >
      ...
      >
        <!-- 获取验证码 -->
        <c-getCode name="addressGetcode" @click="getCodeClick" />
      </c-inputLine>
      <!-- 谷歌验证码 -->
      <c-inputLine
        v-if="OpenGoogle"
...
<script>
import mixin from "@/components/BlockChain-ui-privatization/PC/common-mixin/assets/addressMent/addressMent";
import "@/components/BlockChain-ui-privatization/PC/common-mixin/assets/addressMent/addressMent.styl";
...
.page-addressMent .addressMent-table /deep/.table-head {
  box-sizing: border-box;
  padding: 0 10px !important;
  background: var(--color-BG);
}
.page-addressMent .addressMent-table /deep/.button,
.link,
.icon-btn {
  font-weight: 600;
...
.page-addressMent .addressBox .addressBox-select {
  width: 250px;
}
.addressMent-header {
  font-weight: 600;
