<template>
  <section class="page-leverafeToLoan">
    ...
    <!--</header>-->
    <div class="m-head">
      <div class="title">{{ $t("assets.leverageToLoan.title") }}</div>
    </div>
    <div class="withdraw-details clearfix a-7-bd" v-if="Object.keys(messData).length">
      ...
      <div class="details-right">
        ...
        <div class="coinType">
          <div class="even">
            <c-redio
              :value="broCoin === messData.baseCoin"
              :name="messData.baseCoin"
              @click="setBroCoin"
            />
            <span class="coin" @click="setBroCoin(messData.baseCoin)">{{
              getShowCoin(messData.baseCoin)
            }}</span>
          </div>
          <div class="even">
            <c-redio
              :value="broCoin === messData.quoteCoin"
              :name="messData.quoteCoin"
              @click="setBroCoin"
            />
            <span class="coin" @click="setBroCoin(messData.quoteCoin)">{{
        ...
        <c-button
          ...
          >{{ $t("assets.leverageToLoan.ToLoan") }}</c-button
        >
      </div>
    </div>
    <!-- 最近借贷记录 -->
...
<script>
import mixin from "@/components/BlockChain-ui-privatization/PC/common-mixin/assets/leverageToLoan/leverageToLoan";
import "@/components/BlockChain-ui-privatization/PC/common-mixin/assets/leverageToLoan/leverageToLoan.styl";
...
.page-leverafeToLoan .withdraw-details {
  overflow: hidden;
}
.page-leverafeToLoan .withdraw-details .details-left {
}
.withdraw-table {
  box-sizing: border-box;
  padding: 0 44px;
}
.page-leverafeToLoan .withdraw-details .numberList {
  overflow: hidden;
}
.page-leverafeToLoan .withdraw-details .symbol,
.page-leverafeToLoan .withdraw-details .numberList .numberList-value .text,
.page-leverafeToLoan .withdraw-header {
  color: var(--color-text-primary);
}
.page-leverafeToLoan .withdraw-header {
  margin: 28px 44px;
...
.page-leverafeToLoan .withdraw-details .coinDeat li {
  background: var(--color-BG);
}
.page-leverafeToLoan .withdraw-table /deep/.sub-table {
  background: var(--color-BG);
}
.b-1-cl {
  color: var(--color-1);
