    ...
    <div class="body">
      <div class="table">
        <div class="tr th">
          ...
          <div class="td right">Profit</div>
        </div>
        <div class="tr" v-for="item in detailList">
          <div class="td">{{ item.timeStr }}</div>
          <div class="td right">{{ item.mintAmount }} ORX</div>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    async getMintDetail() {
      ...
      const res = await this.axios({
        url: `${this.opUrl}/mint/detail?uid=${id}`,
        hostType: "def",
        method: "get",
      });
      if (res.code == "0") {
        ...
        this.detailList = list.map((item) => {
          item.timeStr = convertToLocaleTimeStamp(item.mintTime);
