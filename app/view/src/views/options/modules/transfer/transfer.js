...
export default {
  ...
  computed: {
    ...
    showPrecision() {
      ...
      //   return this.marginCoinInfor[this.axiosSymbol] ? this.marginCoinInfor[this.axiosSymbol].marginCoinPrecision : "";
      // }
      return 4;
    },
    // 限制转入
    fundsInStatus() {
      // if (this.marginCoinInfor && this.axiosSymbol) {
      //   return this.marginCoinInfor[this.axiosSymbol] ? this.marginCoinInfor[this.axiosSymbol].fundsInStatus : "";
      // }
      return true;
    },
    // 限制转出
    ...
    transferError() {
      ...
      if (this.transferSide === "1") {
        ...
        if (parseFloat(this.transferValue) > parseFloat(this.optionsAmount)) {
          flag = true;
        }
      }
      return flag;
    ...
    transferWarningText() {
      ...
      if (this.marginCoinInfor && this.axiosSymbol) {
        ...
        num =
          this.transferSide === "1"
            ? cutOut(this.exchangeAmount, 4)
            : cutOut(this.optionsAmount, 4);
    ...
    langTxt() {
      return {
        txt1: this.$t("futures.transfer.optiosAccount"),
      };
    },
    // 方向
    side() {
      const ex = this.$t("futures.transfer.exchangeAccount");
      const co = this.langTxt.txt1;
      let from = "";
      let to = "";
      if (this.transferSide === "1") {
        from = ex;
  ...
  watch: {
    ...
    // 外部传入的方向
    transferSideOutSide(v) {
      console.log("transferSideOutSide", v);
    },
    userInfo(val) {
  ...
  methods: {
    init() {
      this.transferSide = this.transferSideOutSide;
    },
    async getOptionsAssets() {
      ...
      if (res.code === "0") {
        const { locked, normal } = res.data;
        this.optionsAmount = normal;
        this.isAssets = true;
      }
    },
    getData() {
    ...
    async transferFn() {
      ...
      //   url: `${this.opUrl}/account/check?uid=${data.uid}`,
      //   hostType: "def",
      //   method: "get",
      // });
      ...
      if (this.transferSide == "2") {
        res = await this.axios({
          url: `${this.opUrl}/transfer/op_to_ex`,
          hostType: "def",
          method: "post",
          params: data,
        });
        ...
        try {
          res = await this.axios({
            url: `${this.opUrl}/transfer/ex_to_op`,
            hostType: "def",
            method: "post",
            params: data,
          });
      ...
      if (res.code === "0") {
        this.close(true);
        this.$bus.$emit("tip", { text: res.msg });
        this.$bus.$emit("transfer", true);
      } else {
        this.$bus.$emit("tip", {
          text: res.msg,
          type: "error",
    ...
    allTransfer() {
      if (this.transferSide === "1") {
        ...
        if (!this.optionsAmount) {
          return;
        }
        this.transferValue = this.optionsAmount.toString();
      }
    },
  },
};
