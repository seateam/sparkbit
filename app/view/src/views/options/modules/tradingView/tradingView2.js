...
export default {
  ...
  computed: {
    ...
    opWsUrl() {
      return this.$store.state.systemConfig.opWsUrl;
    },
  },
  methods: {
    createWs() {
      const self = this;
      this.socket_g = new WebSocket(this.opWsUrl);
      // this.socket_g = new WebSocket("ws://192.168.1.11:28702/ws");

      // 连接成功时触发
      this.socket_g.onopen = () => {
        window.TradingView.widget && this.createWidget();
        // 订阅K线
        // subscribeKline("btcusdt", "5min");
        // requestKlineData("btcusdt", "1min", 50);
      };

      // 接收到消息时触发
      this.socket_g.onmessage = (event) => {
      ...
      function handleData(data) {
        // 处理接收到的数据
        if (data.event == "REQ") {
          self.$bus.$emit("OPTION_KLINE_DATA_REQ", data);
        } else if (data.event == "SUB") {
          self.$bus.$emit("OPTION_KLINE_DATA_SUB", data);
          self.$bus.$emit("LAST_PRICE", data.data[0]);
          self.currentPrice = data.data[0].price;
        } else if (data.event == "LAST_PRICE") {
          self.currentPrice = data.data.price;
        }
      }
    },
    requestKlineData(symbol, type, pageSize, endIdx = null) {
      ...
      this.socket_g.send(JSON.stringify(requestDataMsg));
    },
    // 发送订阅K线请求
    subscribeKline(symbol, type) {
      const subscribeMsg = {
        path: "/marketKline",
        params: {
          symbol,
      ...
      this.socket_g.send(JSON.stringify(subscribeMsg));
    },
    init() {
      this.createWs();
      // 监听TV 是否是全屏状态
      ...
      return {
        ...
        getBars(
          ...
          if (self.socket_g) {
            ...
            //   sd: "2222",
            // });
            self.requestKlineData(self.symbolName.showSymbol, lastTimeS, 300);
          }
          self.$bus.$off("OPTION_KLINE_DATA_REQ");
          self.$bus.$on("OPTION_KLINE_DATA_REQ", (data) => {
            data.data.sort((a, b) => a.idx - b.idx);
            const Ntime = data.type;
            const symbolType = data.symbol;
            if (symbolType === self.symbolName.symbol && lastTimeS === Ntime) {
              self.fTime = 0;
        ...
        subscribeBars(symbolInfo, resolution, onRealtimeCallback) {
          ...
          const lastTimeS = self.lastTimeS === "Line" ? "1min" : self.lastTimeS;
          if (self.socket_g) {
            self.subscribeKline(self.symbolName.showSymbol, lastTimeS);
          }
          self.$bus.$off("OPTION_KLINE_DATA_SUB");
          self.$bus.$on("OPTION_KLINE_DATA_SUB", (data) => {
            if (data.data.length) {
              const tickData = self.setData(data.data[0]);
    ...
    createWidget() {
      ...
      const redColor = "#FF5F5F";
      const greenColor = this.theme === "Light" ? "#1DC96E" : "#5ceea0";
      const gridColor = this.theme === "Light" ? "rgba(225, 225, 225, 1)" : "rgba(25, 25, 25, 1)";
      const mainColor = this.theme === "Light" ? "#FFFFFF" : "#000000";
      const textColor = this.theme === "Light" ? "rgba(31,31,31,1)" : "rgba(255,255,255,1)";
      // 改，2022.12.30 修改主题色
      window.tvWidget = new F({
        ...
        overrides: {
          ...
          // 改，2022.09.17，改颜色
          "scalesProperties.textColor": textColor, // 图标区域xy轴 文字颜色
          "scalesProperties.lineColor": "rgba(51,51,51,1)", // 图标区域xy轴颜色
          "paneProperties.background": mainColor, // 图标区域 背景色
          "paneProperties.vertGridProperties.color": gridColor, // 图标区域 表格纵轴颜色
          "paneProperties.horzGridProperties.color": gridColor, // 图标区域 表格橫轴颜色
          "paneProperties.vertGridProperties.style": 2, // 图标区域 表格橫轴颜色
          "paneProperties.horzGridProperties.style": 2, // 图标区域 表格橫轴颜色
          ...
          // 柱状图颜色设置
          // getHex(colorMap["u-1-cl"])
          "mainSeriesProperties.style": 2, // 数据列风格
          "mainSeriesProperties.candleStyle.upColor": greenColor,
          // getHex(colorMap["u-4-cl"])
          "mainSeriesProperties.candleStyle.downColor": redColor,
          "mainSeriesProperties.candleStyle.drawWick": true,
          "mainSeriesProperties.candleStyle.drawBorder": true,
          "mainSeriesProperties.candleStyle.borderColor": redColor,
          "mainSeriesProperties.candleStyle.borderUpColor": greenColor,
          "mainSeriesProperties.candleStyle.borderDownColor": redColor,
          "mainSeriesProperties.candleStyle.wickUpColor": greenColor,
          "mainSeriesProperties.candleStyle.wickDownColor": redColor,
