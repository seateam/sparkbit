  ...
  watch: {
    ...
    symbolCurrent(val, oldVla) {
      if (val && this.symbolAll) {
        if (this.checkStopOrder) {
          this.checkStopOrder(); // 判断是否显示条件单
        }
        this.getJz();
        ...
        // 改，2022.12.05 新增用于收集是否展示多图传参
        // 改，2022.12.07 使用 this.calTradeType 做默认值
        const type = this.$route.query.type || this.calTradeType;
        this.$store.commit("changeTradeType", { type });
