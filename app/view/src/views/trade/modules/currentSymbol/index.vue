...
<template>
  <div class="currentSymbol a-21-bg a-3-bd">
    ...
    <!-- 改，2022.09.21，新增 -->
    <!-- 24小时行情 -->
    <div style="position: absolute; z-index: 10; top: 40px" v-show="calCurKlLint.isShowMarket">
...
export default {
  ...
  data() {
    return {
      // 是否显示市场
      isShowMarket: this.currentItem.isShowMarket,
    };
  },
  mounted() {
    this.init();
  ...
  computed: {
    ...
    // 获取当前k线图索引
    kLineIndex() {
      return this.currentItem.kLineIndex;
    },
    // 当前的K线数据
    calCurKlLint() {
      return this.calKLineDataItem(this.kLineIndex);
    },
  },
  // 改，2022.09.21，新增
  methods: {
    ...mapMutations(["updateShowMarket"]),
    // 显示 市场
    showMarket() {
      // this.isShowMarket = !this.isShowMarket;
      // 改，2022/12/02 使用vuex数据
      this.updateShowMarket({
        index: this.kLineIndex,
        isShow: this.calCurKlLint.isShowMarket,
      });
    },
    // 关闭市场
    onClickOutside() {
