...
export default {
  ...
  computed: {
    // 改，2022.12.02 symbolCurrent转computed
    symbolCurrent() {
  ...
  methods: {
    init() {
      ...
      this.$bus.$on("SYMBOL_CURRENT", (val) => {
        if (this.symbolCurrent !== val) {
          ...
          // 改，2022.12.02 使用vuex管理
          // this.symbolCurrent = val;
          this.$store.commit("updateKey2Val", { index: this.currentItem.kLineIndex, key: "symbolCurrent", value: val });
          if (window.tvWidget) {
    ...
    createWidget() {
      ...
      const Positive = this.theme === "Light" ? "#1DC96E" : "#5ceea0";
