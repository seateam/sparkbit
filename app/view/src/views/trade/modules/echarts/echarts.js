...
export default {
  ...
  methods: {
    init() {
      ...
      this.$bus.$on('ECHARTS_DATA', (data) => {
        // console.log(data, "ECHARTS_DATA data");
        if (data) {
    ...
    initEachart() {
      ...
      this.myEcharts = window.echarts.init(document.getElementById(this.echartsId));
      // 绘制图表
      this.myEcharts.setOption({
        animation: false,
        tooltip: {
  ...
  computed: {
    ...
    echartsId() {
      return `myEcharts${Math.floor(Math.random() * 100000)}`;
    },
  },
};
