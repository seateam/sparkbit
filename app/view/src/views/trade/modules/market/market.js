...
export default {
  ...
  data() {
    return {
      ...
      listfilterVal: null,
      // 自选币对
      mySymbolList: this.currentItem.mySymbol || [],
  ...
  computed: {
    // 改，2022.12.02 新增 getters
    ...mapGetters(["calKLineDataItem"]),
    // 改，2022.12.02 symbolCurrent
    symbolCurrent() {
      return this.currentItem.symbolCurrent;
    },
    marketCurrent() {
      return this.currentItem.marketCurrent;
    },
    marketClass() {
  ...
  methods: {
    // 改，2022.12.02 新增操作当前k线数据方法
    ...mapMutations(["updateMarket", "updateKline"]),
    init() {
      this.$bus.$on("extend", () => {
        this.shrinkBlock();
      });
    ...
    symbolTitle(data) {
      return [
        `${getCoinShowName(data.symbol, this.coinList)}/${getCoinShowName(
          data.unit,
          this.coinList,
        )}`,
      ];
      ...
      keyarr.forEach((item) => {
        ...
        if (this.marketDataList[item]) {
          if (isShow || this.listfilterVal || this.marketCurrent === "myMarket") {
            ...
            if (newcoinFlag === 3 && this.coinList[symbol] && !this.coinList[symbol].isOvercharge) {
    ...
    switchSymbol(data) {
      // 新增
      const { kLineIndex } = this.currentItem;
      if (this.isAdd) {
        const curItem = this.calKLineDataItem(kLineIndex);
        curItem.marketCurrent = "USDT";
        curItem.symbolCurrent = data;
        curItem.isShowMarket = false;
        this.updateKline({ index: kLineIndex, data: curItem });
        return;
      }
      // 判断 tradingview 已经初始化完成
      // if (window.tvWidget) {
      // this.symbolCurrent = data;
      this.$store.commit("updateKey2Val", { index: kLineIndex, key: "symbolCurrent", value: data });
      this.$store.commit("updateKey2Val", { index: kLineIndex, key: "isShowMarket", value: false });
