...
export default {
  ...
  data() {
    return {
      ...
      MywebSocket: null,
      // // 当前市场
      // marketCurrent: this.currentItem.marketCurrent,
      // // 当前币对
      // symbolCurrent: this.currentItem.symbolCurrent,
      // 当前币对列表
      symbolCurrentList: [],
      // 深度级别
      depthClasses: "0",
      ...
      etfPrice: "--",
      lTime: null,
      lastTimeS: this.currentItem.lastTimeS || "",
    };
  },
  computed: {
    // 改，2022.12.06 改获取当前市场的方式
    ...mapGetters(['calMarketCurrent']),
    // 改，2022.12.02 symbolCurrent
    symbolCurrent() {
      return this.currentItem.symbolCurrent;
    },
    marketCurrent() {
      return this.calMarketCurrent(this.currentItem.kLineIndex);
    },
    fundRate() {
    ...
    ...mapState({
      baseInfo({ baseData }) {
        ...
        // 改，2022.12.02 转vuex管理
        // // 获取当前市场
        // this.marketCurrent = this.currentItem.marketCurrent;
        // // 获取当前币对
        // this.symbolCurrent = this.currentItem.symbolCurrent;
        return baseData;
      },
    ...
    tradeLimitAlertText() {
      ...
      if (this.tradeLimitData) {
        const symbol = this.symbolCurrent.split("/")[0];
        const {
        ...
        // 单日最大买入量为：
        const txt1 = `${
          this.$t("tradeLimt.tradeLimit_text_everyDayBuy")
          + fixD(tradeSymbolBuyLimit, this.fixValue.volumeFix)
          + symbol
          + this.$t("tradeLimt.tradeLimit_text_everyDayBuy_end")
        }，`;
        // 单日最大卖出量为：
        const txt2 = `${
          this.$t("tradeLimt.tradeLimit_text_everyDaySell")
          + fixD(tradeSymbolSellLimit, this.fixValue.volumeFix)
          + symbol
          + this.$t("tradeLimt.tradeLimit_text_everyDaySell_end")
        }`;
        // 未限制买入交易
    ...
    symbolList() {
      ...
      if (this.marketCurrent === "myMarket") {
        const mySymbol = this.currentItem.mySymbol || [];
        const marketList = {};
        if (mySymbol.length) {
  ...
  methods: {
    ...
    init() {
      this.getJz();
      this.routeSymbol = this.currentItem.symbolCurrent.replace("/", "_");
      if (this.routeSymbol) {
    ...
    listenBusEvent() {
      // 监听 市场切换
      this.$bus.$off("ON_MARKET_SWITCH");
      this.$bus.$on("ON_MARKET_SWITCH", (data) => {
        // this.marketCurrent = data;
        this.$store.commit("updateKey2Val", {
          index: this.currentItem.kLineIndex,
          key: "marketCurrent",
          value: data,
        });
      });
      // 监听 币对切换
      this.$bus.$off("ON_SYMBOL_SWITCH");
      this.$bus.$on("ON_SYMBOL_SWITCH", (data) => {
        // this.symbolCurrent = data;
        this.$store.commit("updateKey2Val", { index: this.currentItem.kLineIndex, key: "symbolCurrent", value: data });
