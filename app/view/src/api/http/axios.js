...
import { sign, encrypt } from "@/utils/securityUtil.js";
import bus from '../bus';
...
const request = function request(prefix, url, headers, params, method, hostType, resolve, reject, fullUrl, signParams = false) {
  let _url = `${prefix}${url}`;
  if (fullUrl) {
    _url = fullUrl;
  }
  ...
  let signDate = '';
  if (signParams) {
    signStr = sign(formatParams(params), timestamp, `/${url}`);
    signHeader = { ...formatHeaders(headers), sign: signStr, timestamp };
    signDate = encrypt(formatParams(params));
  }
  return axios({
    url: _url, // exchange-web-api线上  vue-api本地
    headers: !signParams ? formatHeaders(headers) : signHeader,
    data: !signParams ? formatParams(params) : signDate,
    method: method || 'post',
  }).then((data) => {
    if (typeof data.data.code !== 'undefined') {
      if (data.data.code.toString() === '110009') {
        setCookie("token", "");
      }
...
const http = ({
  url, headers, params, method, hostType, fullUrl, signParams,
}) => {
  let prefix = '';
  switch (hostType) {
    case 'otc':
      prefix = '/fe-otc-api/'; // 法币
      break;
    case 'co':
      prefix = '/fe-co-api/'; // 合约
      break;
    case 'debit':
      prefix = '/fe-debit-api/'; // 上传
      break;
    case 'upload':
  ...
  return new Promise((resolve, reject) => {
    if (!window.secur) {
      ...
      window.security.init(msec, (info) => {
        window.secur = info;
        request(prefix, url, headers, params, method, hostType, resolve, reject, fullUrl, signParams);
      });
    } else {
      request(prefix, url, headers, params, method, hostType, resolve, reject, fullUrl, signParams);
    }
