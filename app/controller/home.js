...
class HomeController extends Controller {
  ...
  async getPublicInfo(v) {
    ...
    const pub = this.fsPublicInfo(
      path.join(this.config.staticPath, `${domainArr[this.fileName].fileName}.json`)
    );
    if (pub.isHave) {
      this.publicInfo = pub.data;
      // 1.2.2 如果无此文件 则发起请求
    } else {
      await this.ctx.service.publictInfo.getdata(
        domainArr[this.fileName],
        (data) => {
          otherArr[this.fileName].time = new Date().getTime();
          otherArr[this.fileName].sendLoading = false;
          if (data.code === 0) {
            this.publicInfo = data.data;
          }
        },
    ...
    if (v) {
      ...
      this.ctx.service.publictInfo.getdata(
        domainArr[this.fileName],
        () => {
          otherArr[this.fileName].time = new Date().getTime();
          otherArr[this.fileName].sendLoading = false;
  ...
  async index() {
    ...
    // 改，2022.08.14，加condole
    console.log("home.js", "domain", domain);
    ...
    if (domainArr[fileName]) {
      ...
      ctx.service.publictInfo.getdata(
        domainArr[fileName],
        () => {
    ...
    if (JSON.stringify(this.publicInfo) !== "{}") {
      ...
      if (publicData && publicData.msg && publicData.msg.iconUrl) {
      ...
      // 改，2022.08.14，加condole
      console.log("home.js", "this.lan", this.lan);

      // 设置 eCharts地址
      if (this.lan !== "zh_CN") {
        renderObject.eChartsUrl =
      ...
      if (publicData && publicData.seo && publicData.seo[this.lan]) {
        ...
        const { description, keywords, pageContent, title } = seoLan;
  ...
  async setLan(domain) {
    ...
    if (this.publicInfo && this.publicInfo.data && this.publicInfo.data.lan) {
