let security_components = null;
const security_interval = null;
let security_last_device_id = '';
let security_user_identity = '';
function GetCookieDomain() { let host = location.hostname; const ip = /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/; if (ip.test(host) == true || host == 'localhost') return host; const regex = /([^]*).*/; const match = host.match(regex); if (typeof match !== "undefined" && match != null)host = match[1]; if (typeof host !== "undefined" && host != null) { const strAry = host.split("."); if (strAry.length > 1) { host = `${strAry[strAry.length - 2]}.${strAry[strAry.length - 1]}`; } } return `.${host}`; }
...
function setCookie(e, o, i) { void 0 != i && i != null || (i = 120), typeof i !== "number" && (i = 120); const t = 60 * i * 1e3; const n = new Date(); n.setTime(n.getTime() + t + 288e5), document.cookie = `${e}=${o};path=/;expires=${n.toUTCString()};domain=${GetCookieDomain()}`; }
function ajax_method(e, n, t) { const o = new XMLHttpRequest(); t == "get" ? (n && (e += "?", e += n), o.open(t, e), o.send()) : (o.open(t, e), o.setRequestHeader("Content-type", "application/x-www-form-urlencoded"), n ? o.send(n) : o.send()); }
(function (name, context, definition) {
  ...
  const each = function (r, o) { if (Array.prototype.forEach && r.forEach === Array.prototype.forEach)r.forEach(o); else if (r.length === +r.length) for (let a = 0, e = r.length; a < e; a++)o(r[a], a, r); else for (const f in r)r.hasOwnProperty(f) && o(r[f], f, r); };

  const map = function (a, p) { const r = []; return a == null ? r : Array.prototype.map && a.map === Array.prototype.map ? a.map(p) : (each(a, (a, t, n) => { r.push(p(a, t, n)); }), r); };

  const extendSoft = function (r, n) { if (n == null) return r; let t; let e; for (e in n)(t = n[e]) == null || Object.prototype.hasOwnProperty.call(r, e) || (r[e] = t); return r; };

  // https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/enumerateDevices
  const enumerateDevicesKey = function (e, i) { if (!isEnumerateDevicesSupported()) return e(i.NOT_AVAILABLE); navigator.mediaDevices.enumerateDevices().then((i) => { e(i.map(e => `id=${e.deviceId};gid=${e.groupId};${e.kind};${e.label}`)); }).catch((i) => { e(i); }); };
  ...
  const jsFontsKey = function (done, options) {
    ...
    fontList = fontList.filter((font, position) => fontList.indexOf(font) === position);
  ...
  const security = function (options) {
    throw new Error("'new Fingerprint()' is deprecated, see https://github.com/Valve/fingerprintjs2#upgrade-guide-from-182-to-200");
  };

  security.get = function (options, callback) {
    if (!callback) {
      callback = options;
      options = {};
    } else if (!options) {
  ...
  security.init = function (config, callback) {
    if (!config) {
      config = {
        org: "",
        server: "",
    ...
    if (security_components) {
      object_data.org = config.org || '';
      object_data.timestamp = new Date().getTime();
      let device_id = x64hash128(map(security_components, (component) => {
        if (Object.prototype.toString.call(component.value) == "[object Array]") {
          component.value = component.value.join(',');
        }
        object_data[component.key] = component.value;
        return component.value;
      }).join('~~~'), 31);
      object_data.device = device_id;
      const package_data = JSON.stringify(object_data);
      if (config.sendRemote && (security_user_identity != object_data.identity || device_id != security_last_device_id)) {
        let _query = '';
        for (const index in object_data) {
          _query += `${index}=${object_data[index]}&`;
        }
        ajax_method(config.server, _query, 'post');
        security_user_identity = object_data.identity;
        device_id = security_last_device_id;
      }
      // callback(object_data, security_components)
      if (callback) {
    ...
    const options = {};
    return security.get(options, (components) => {
      const newComponents = [];
      ...
      security_components = newComponents;
      object_data.org = config.org || '';
      object_data.timestamp = new Date().getTime();
      const device_id = x64hash128(map(newComponents, (component) => {
        if (Object.prototype.toString.call(component.value) == "[object Array]") {
          component.value = component.value.join(',');
        }
        object_data[component.key] = component.value;
      ...
      security_last_device_id = device_id;
      object_data.device = device_id;
      security_user_identity = object_data.identity;
      ...
      // callback(object_data, newComponents)
      if (callback) {
        callback(object_data);
      }
    });
  };
  security.x64hash128 = x64hash128;
  security.VERSION = '2.0.6';
  return security;
}));
